...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
	oplus_kbase_notify_gpu_inode((void *)inode);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
const struct oplus_pm_core_linux_operations oplus_pm_core_operations = {
	.open = kbase_open,
	.release = kbase_release,
	.handshake = kbase_api_handshake,
	.set_flags = kbase_api_set_flags,
};
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
	oplus_kbase_set_operations(&oplus_pm_core_operations);
	oplus_kbase_fun_fp_init();
#endif
	return err;
}
