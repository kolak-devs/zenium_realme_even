...
union kbase_ioctl_kinstr_prfcnt_setup_kernel;
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
/**
 * kbase_kinstr_prfcnt_setup_kernel() - Set up a new hardware counter reader client for powermode kernel object,no userspace data copy.
 * @kinstr_ctx: Non-NULL pointer to the kinstr_prfcnt context.
 * @setup:      Non-NULL pointer to the hwcnt reader configuration.
 *
 * Start a session between a user client and the kinstr_prfcnt component.
...
int kbase_kinstr_prfcnt_setup_kernel(struct kbase_kinstr_prfcnt_context *kinstr_ctx,
			      union kbase_ioctl_kinstr_prfcnt_setup_kernel *setup);


/**
 * kbase_kinstr_prfcnt_enum_info_kernel - Enumerate performance counter information for powermode kernel object,no userspace data copy.
 * @kinstr_ctx: Non-NULL pointer to the kinstr_prfcnt context.
 * @enum_info:  Non-NULL pointer to the enumeration information.
 *
 * Enumerate which counter blocks and banks exist, and what counters are
