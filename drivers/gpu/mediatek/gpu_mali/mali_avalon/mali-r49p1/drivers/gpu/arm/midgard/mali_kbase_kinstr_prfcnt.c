...
extern int (*oplus_kbase_api_kinstr_prfcnt_cmd_fp)(void *cli, void *control_cmd);
extern int (*oplus_kbase_api_kinstr_prfcnt_get_sample_fp)(void *cli, void *sample_access);
extern int (*oplus_kbase_api_kinstr_prfcnt_put_sample_fp)(void *cli, void *sample_access);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
 int oplus_api_kbase_api_kinstr_prfcnt_cmd (void *cli, void *control_cmd)
 {
	int rcode = 0;	
	struct kbase_kinstr_prfcnt_client *client = (struct kbase_kinstr_prfcnt_client *) cli;
	struct prfcnt_control_cmd *cmd = (struct prfcnt_control_cmd *)control_cmd;
	rcode = kbasep_kinstr_prfcnt_cmd(client, cmd);

	if (cmd->cmd == PRFCNT_CONTROL_CMD_STOP) {
		mutex_lock(&client->kinstr_ctx->lock);
		WARN_ON(client->kinstr_ctx->client_count == 0);
		if (client->kinstr_ctx->client_count > 0)
			client->kinstr_ctx->client_count--;
		list_del(&client->node);
		mutex_unlock(&client->kinstr_ctx->lock);

		kbasep_kinstr_prfcnt_client_destroy(client);
	}
	return rcode;
 }

int oplus_api_kbase_api_kinstr_prfcnt_get_sample (void *cli, void *sample_access)
 {
	int rcode = 0;	
	struct kbase_kinstr_prfcnt_client *client = (struct kbase_kinstr_prfcnt_client *) cli;
	rcode = kbasep_kinstr_prfcnt_get_sample(client, (struct prfcnt_sample_access *)sample_access);
	return rcode;
 }

 int oplus_api_kbase_api_kinstr_prfcnt_put_sample (void *cli, void *sample_access)
 {
	int rcode = 0;	
	struct kbase_kinstr_prfcnt_client *client = (struct kbase_kinstr_prfcnt_client *) cli;
	rcode = kbasep_kinstr_prfcnt_put_sample(client, (struct prfcnt_sample_access *)sample_access);
	return rcode;
 }
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
	oplus_mali_notify_prfcnt_client_dump(0);
#endif
}
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
	oplus_kbase_api_kinstr_prfcnt_cmd_fp = oplus_api_kbase_api_kinstr_prfcnt_cmd;
	oplus_kbase_api_kinstr_prfcnt_get_sample_fp = oplus_api_kbase_api_kinstr_prfcnt_get_sample;
	oplus_kbase_api_kinstr_prfcnt_put_sample_fp = oplus_api_kbase_api_kinstr_prfcnt_put_sample;
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_POWERMODEL)
int kbase_kinstr_prfcnt_setup_kernel(struct kbase_kinstr_prfcnt_context *kinstr_ctx,
			      union kbase_ioctl_kinstr_prfcnt_setup_kernel *setup_kernel)
{
	int err;
	size_t item_count, max_item_count;
	size_t bytes;
	...
	const size_t max_bytes = 32 * sizeof(*req_arr);
	const struct kbase_hwcnt_metadata *metadata;
	union kbase_ioctl_kinstr_prfcnt_setup setup = {0};

	setup.in.request_item_count = setup_kernel->in.request_item_count;
	setup.in.request_item_size = setup_kernel->in.request_item_size;
	setup.in.requests_ptr = setup_kernel->in.requests_ptr;

	if (!kinstr_ctx || !setup_kernel)
		return -EINVAL;
	...
	req_arr = (void *)(setup.in.requests_ptr);
	...
	setup_kernel->out.prfcnt_metadata_item_size = sizeof(struct prfcnt_metadata);
	setup_kernel->out.prfcnt_mmap_size_bytes = cli->sample_size * cli->sample_count;
	setup_kernel->out.cli_ptr = (__u64)cli;
	setup_kernel->out.cli_prfcnt_user_buf = (__u64)cli->sample_arr.user_buf;
...
static int kbasep_kinstr_prfcnt_enum_info_list_kernel(
	struct kbase_kinstr_prfcnt_context *kinstr_ctx,
	struct kbase_ioctl_kinstr_prfcnt_enum_info *enum_info)
{
	struct prfcnt_enum_item *prfcnt_item_arr;
	...
	if (!err) {
		unsigned long bytes = enum_info->info_item_count * sizeof(*prfcnt_item_arr);
		memcpy((void *)(enum_info->info_list_ptr), prfcnt_item_arr, bytes);
	}
...
int kbase_kinstr_prfcnt_enum_info_kernel(
	struct kbase_kinstr_prfcnt_context *kinstr_ctx,
	struct kbase_ioctl_kinstr_prfcnt_enum_info *enum_info)
{
	int err;
	...
	if (!enum_info->info_list_ptr)
		...
		err = kbasep_kinstr_prfcnt_enum_info_list_kernel(kinstr_ctx,
							  enum_info);
